    ...
    async def call_tool(name, arguments):
        try:
            if name == 'mochi_state':
                ...
                text = res.get('story', res.get('msg', '失败'))
            else:
...
if __name__ == '__main__':
    uvicorn.run(starlette_app, host='0.0.0.0', port=8766)
            Tool(name='pet_adopt', description='领养宠物（不花金币，AI帮写入state）', inputSchema={'type':'object','properties':{'name':{'type':'string','description':'宠物名字'},'emoji':{'type':'string','description':'宠物emoji'}},'required':['name','emoji']}),
            Tool(name='pet_to_school', description='送宠物去上学', inputSchema={'type':'object','properties':{}}),
            Tool(name='pet_home', description='接宠物回家', inputSchema={'type':'object','properties':{}}),
            Tool(name='pet_school_event', description='触发今日学校剧情（每天一次）', inputSchema={'type':'object','properties':{}}),
import asyncio, json, urllib.request, urllib.parse
from mcp.server import Server
from mcp.server.sse import SseServerTransport
from starlette.applications import Starlette
